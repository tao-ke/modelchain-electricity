...
class PriceEvolutionModel:
  ...
  #申报容量
def simple_fixed_sum_offers_vec(plant_cap, dist_demand, cap_ratio_upper, cap_ratio_lower, Q_lb, target_sum, n_plants, n_sims):
    ...
    initial_offers = np.random.triangular(lower, upper, upper, size=(n_sims, n_plants))
...
def monte_carlo_simulation(
    ...
    n_plants = max(1, int(round(total_capacity / avg_plant_size)))    #站点数量。
    ...
    if target_bid is not None:
        ...
        competitor_bids = np.random.normal(bid_mean, bid_std_ratio * bid_mean ,size=(n_sims, n_plants - 1))
        #competitor_bids = np.random.triangular(3.5, bid_mean, 15.0, n_plants - 1) 
        #competitor_bids = np.random.uniform(bid_mean - 2*bid_std_ratio*bid_mean, bid_mean + 2*bid_std_ratio*bid_mean, n_plants - 1)  # 使用均匀分布生成竞争对手报价
        #competitor_bids = np.random.uniform(bid_mean - 2*bid_std_ratio*bid_mean, bid_mean + 2*bid_std_ratio*bid_mean, size=(n_sims, n_plants - 1))
        ...
        competitor_k = np.random.uniform(k_mean - k_std, k_mean + k_std, size=(n_sims, n_plants - 1))
        competitor_k = np.maximum(competitor_k, 0.1)
        ...
        competitor_m = np.random.normal(adjusted_m_mean, m_std, size=(n_sims, n_plants - 1))
        competitor_m = np.clip(competitor_m, 1.2, m_max)
        m = np.column_stack([np.full((n_sims, 1), adjusted_m_mean), competitor_m])
    else:
        bids = np.random.normal(bid_mean, bid_std_ratio * bid_mean, size=(n_sims, n_plants))
        bids = np.clip(bids, price_min, price_max)
        
        k = np.random.uniform(k_mean - k_std, k_mean + k_std, size=(n_sims, n_plants))
        k = np.maximum(k, 0.1)
        
        m = np.random.normal(adjusted_m_mean, m_std, size=(n_sims, n_plants))
        m = np.clip(m, 1.2, m_max)

    # 日响应次数
    N_day = np.random.normal(N_day_mean, N_day_std, size=(n_sims, n_plants))
    N_day = np.maximum(N_day, 0)
    # 调节深度
    d = np.random.normal(d_mean, d_std, size=(n_sims, n_plants))
    d = np.clip(d, 0, 1)
    # 申报容量 (严格按细则公式)
    offers = simple_fixed_sum_offers_vec(plant_cap, dist_demand, cap_ratio_upper, cap_ratio_lower, Q_lb, total_capacity, n_plants, n_sims)
    
    # ----- 归一化排序性能指标 P_i -----
    k_max_arr = np.max(k, axis=1, keepdims=True)
    k_max_arr = np.maximum(k_max_arr, 1e-6)
    P = k / k_max_arr
    
     # ----- 边际替代率系数 Fm 计算 (仅储能) -----
    internal_price = bids / P
    sorted_idx_Fm = np.argsort(internal_price, axis=1)
    
    ratios = offers / dist_demand
    ratios_sorted = np.take_along_axis(ratios, sorted_idx_Fm, axis=1)
    cum_ratio_sorted = np.cumsum(ratios_sorted, axis=1)
    
    Fm_sorted = np.where(cum_ratio_sorted <= U_x, 
                         U_y - (U_y - 1) * (cum_ratio_sorted / U_x),
                         np.maximum(0, 1 - (cum_ratio_sorted - U_x) / (1 - U_x)))
                         
    Fm = np.zeros_like(Fm_sorted)
    np.put_along_axis(Fm, sorted_idx_Fm, Fm_sorted, axis=1)
    safe_Fm = np.where(Fm > 0, Fm, 1e-9)
    
    # ----- 排序价格 -----
    sort_price = bids / (P * safe_Fm)
    
    # ----- 市场出清 -----
    order = np.argsort(sort_price, axis=1)
    offers_sorted = np.take_along_axis(offers, order, axis=1)
    
    accumulated = np.cumsum(offers_sorted, axis=1)
    prev_accumulated = accumulated - offers_sorted
    
    assigned_sorted = np.clip(dist_demand - prev_accumulated, 0, offers_sorted)
    
    cleared_cap = np.zeros_like(assigned_sorted)
    np.put_along_axis(cleared_cap, order, assigned_sorted, axis=1)
    
    sort_price_sorted = np.take_along_axis(sort_price, order, axis=1)
    
    # ----- 出清价格 -----
    is_assigned = assigned_sorted > 0
    # 在每次模拟中，找到被分配容量的最高排序价格作为出清价格
    clear_price_per_sim = np.max(np.where(is_assigned, sort_price_sorted, 0), axis=1)
    clear_price = np.minimum(clear_price_per_sim, price_max)
    
    # ----- 计算收益 -----
    daily_rev = cleared_cap * N_day * d * clear_price[:, None] * m / np.float64(10000.0)
    annual_rev = daily_rev * 365.0
    
    total_annual_rev = np.sum(annual_rev, axis=1)
    # ----- 月收益（万元/MW/月） -----
    avg_revenues = total_annual_rev / total_capacity / 12.0
    ...
    if return_detailed_stats:
        ...
        if target_bid is not None:
            ...
            # 严格计算 0号机 容量 × 每天频次 × 深度 × 统一出清价 × 自己的 m值
            target_daily_rev = target_cleared * N_day[:, 0] * d[:, 0] * clear_price * m[:, 0] / np.float64(10000.0)
            target_monthly_rev = (target_daily_rev * 365.0) / plant_cap / 12.0
            
            detailed_stats['target_monthly_revs'] = target_monthly_rev.tolist()
...
def run_clearing_boundary_analysis(
    total_capacity, max_demand, dist_demand_ratio,
    bid_mean, bid_std_ratio,
    k_mean, k_std, m_mean, m_std, m_competition_factor, m_max,
    N_day_mean, N_day_std, d_mean, d_std,
    U_x, U_y, cap_ratio_upper, cap_ratio_lower, Q_lb,
    price_min, price_max
):
    """
    出清边界分析：
    1. 统计多次模拟的真实申报量。
    2. 基于“内部排序价格”找到 P10/P50/P90 核心决策线。
    """
    dist_demand = max_demand * dist_demand_ratio
    
    avg_plant_size = 150.0  
    n_plants = max(1, int(round(total_capacity / avg_plant_size)))
    plant_cap = total_capacity / n_plants
    
    boundary_sims = 200  # 运行 200 次模拟获取充分的散点
    
    all_k = []
    all_bids = []
    
    total_declared_list = []      # 每次模拟的真实总申报量
    margin_internal_list = []     # 每次模拟的边际“内部价格”
    k_max_list = []               # 每次模拟的 k_max

    for sim in range(boundary_sims):
        # 1. 随机生成市场特征
        #bids = np.random.normal(bid_mean, bid_mean * bid_std_ratio, n_plants)
        bids = np.random.uniform(price_min, price_max, n_plants)
        bids = np.clip(bids, price_min, price_max)
        
        # k 值
        k = np.random.uniform(max(1.0, k_mean - 1.5 * k_std), k_mean + 1.5 * k_std, n_plants)
        #k = np.random.normal(k_mean, k_std, n_plants)
        k = np.clip(k, 1.0, 3.0)
        k_max = np.max(k)
        if k_max < 1e-6: k_max = 1e-6
        ...
        lower = np.minimum(lower_candidate, dist_demand * cap_ratio_lower)
        lower = np.minimum(lower, upper)
        offers = np.random.triangular(lower, upper, upper, n_plants)
        
        current_total_decl = np.sum(offers)
        total_declared_list.append(current_total_decl)
        
        # 2. 纯粹利用“内部价格”决定出清顺序
        internal_price = bids / P
        order = np.argsort(internal_price)
        
        accumulated = 0.0
        last_internal_price = 0.0
        
        for idx in order:
            if offers[idx] <= 0: continue
            
            accumulated += offers[idx]
            last_internal_price = internal_price[idx] # 刷新最后中标者的内部价格
            
            if accumulated >= dist_demand - 1e-6:
                break
                
        # 当容量能填满需求，就记录当轮的出清价格
        if accumulated >= dist_demand - 1e-6:
            margin_internal_list.append(last_internal_price)
    ...
    avg_declared = np.mean(total_declared_list)
    avg_k_max = np.mean(k_max_list)
    
    fig, ax = plt.subplots(figsize=(10, 6))
    
    # 3. 如果市场没有过剩（所有边际线都没找到）
    if len(margin_internal_list) == 0:
        ax.scatter(all_k, all_bids, c='green', marker='o', s=30, alpha=0.5, label='全部中标 (无淘汰)')
        ax.set_title(f'市场供不应求：平均真实申报 {avg_declared:.0f}MW < 需求 {dist_demand:.0f}MW\n所有报价和K值100%全额中标', fontsize=14, color='red')
        ax.set_xlabel('排序性能指标 K值', fontsize=12)
        ax.set_ylabel('申报价格 (元/MW)', fontsize=12)
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        
        summary = f"""
        ### ⚠️ 当前配置无竞争，所以电站中标
        - 配置的全网独立储能容量： **{total_capacity} MW**
        - **但是真实平均总申报量仅为：{avg_declared:.0f} MW**
        - 而配额需求为：**{dist_demand:.0f} MW**
        
        由于“申报卖出”的总容量达不到总需求（{avg_declared:.0f} < {dist_demand:.0f}），直接中标。
        """
        return fig, summary
        
    # 4. 市场发生淘汰，计算稳定决策线
    p50_internal = np.percentile(margin_internal_list, 50)
    p10_internal = np.percentile(margin_internal_list, 10) # 悲观情况线
    p90_internal = np.percentile(margin_internal_list, 90) # 乐观情况线
    
    # 只按照 P50中位数线 强行给所有散点染色
    # 规则：只要 bid <= p50_internal * (K / average_Kmax) 就中标
    p50_win_mask = all_bids <= (p50_internal * (all_k / avg_k_max))
    
    ax.scatter(all_k[p50_win_mask], all_bids[p50_win_mask], c='green', marker='o', s=30, alpha=0.5, label='P50大概率中标区')
    ax.scatter(all_k[~p50_win_mask], all_bids[~p50_win_mask], c='red', marker='x', s=30, alpha=0.5, label='P50大概率落选区')
    
    # 提取数学斜率
    slope = p50_internal / avg_k_max
    
    # P50 的数学公式文本
    formula_text = (
        f"P50公式: 报价(Y) = {slope:.3f} * 性能(K)\n"
        f"(斜率 = {slope:.3f})"
    )
    ...
    # 2. 计算与最高报价（price_max，例如15元）的交点
    intersect_y = price_max
    intersect_x = intersect_y / slope
    
    # 3. 绘制交点（如果交点在图表可视范围内）
    ax.axhline(y=intersect_y, color='gray', linestyle=':', alpha=0.5)
    ax.axvline(x=intersect_x, color='gray', linestyle=':', alpha=0.5)
    
    ax.plot(intersect_x, intersect_y, 'b*', markersize=15, 
            label=f'报顶价({intersect_y}元)的 P50 及格 K值')
    
    ax.annotate(f'K需达到 {intersect_x:.2f}', 
                xy=(intersect_x, intersect_y),
                xytext=(15, -15), textcoords='offset points', 
                color='blue', weight='bold', fontsize=11,
                arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0.2', color='blue'))
    
    # 绘制决策刻度线 
    k_range = np.linspace(np.min(all_k)*0.95, np.max(all_k)*1.05, 100)
    
    line_p50 = p50_internal * (k_range / avg_k_max)
    line_p10 = p10_internal * (k_range / avg_k_max)
    line_p90 = p90_internal * (k_range / avg_k_max)
    
    ax.plot(k_range, line_p50, 'k-', linewidth=3, label=f'P50基准决策线')
    ax.plot(k_range, line_p10, 'g--', linewidth=2, label=f'P10安全区界限 (保守)')
    ax.plot(k_range, line_p90, 'r--', linewidth=2, label=f'P90危险区界限 (激进)')
    
    ax.fill_between(k_range, price_min - 1, line_p10, color='darkgreen', alpha=0.1, label='极高概率中标区')
    ax.fill_between(k_range, line_p10, line_p90, color='yellow', alpha=0.2, label='不确定区')
    ax.fill_between(k_range, line_p90, price_max + 1, color='darkred', alpha=0.1, label='极易被淘汰区')

    ax.set_xlim(np.min(all_k)*0.95, np.max(all_k)*1.05)
    ax.set_ylim(price_min - 0.5, price_max + 0.5)
    
    ax.set_title(f'决策边界矩阵图', fontsize=15)
    ax.set_xlabel('排序性能指标 K 值', fontsize=12)
    ax.set_ylabel('计划主推报价 (元/MW)', fontsize=12)
    ax.grid(True, alpha=0.3)
    ...
    **市场态势：**
    - 电站总容量：{total_capacity} MW
    - **每天参与排队竞价的真实平均量：{avg_declared:.0f} MW**
    - 市场容量需求配额池：{dist_demand:.0f} MW
    - 竞争饱和度 (供需比)：**{avg_declared/dist_demand:.2f}** 

    **决策图深度解读 (去除 $F_m$ )：**
    根据推演，是否能中标**只取决于内部价格（报价 ÷ K相对值）**。
    - **最下方/绿色底色**：绝对安全区。
    - **正中间/黄色走廊**：风险区。是否中标受制于当天其它对手是否犯错报错。K值越高，越向上倾斜。
    - **最上方/红色底色**：中标失败区。报价非常高或K值极低，不会中标。
    """
...
def calc_win_rate_matrix_data(
    ...
    base_kwargs = dict(
        ...
        m_competition_factor=m_competition_factor, m_max=m_max,
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        for res in executor.map(_calc_win_rate_worker, tasks):
            results.append(res)
...
def plot_win_rate_matrix_data(df, metric_choice):
    ...
    fig.subplots_adjust(hspace=0.4, wspace=0.15, top=0.92, bottom=0.08, left=0.08, right=0.95)
    
    title_metric = '中标概率' if metric_choice == '中标概率' else '期望收益 (万元/MW/月)'
    fig.suptitle(f'【全解空间】各维度交叉{title_metric}矩阵', fontsize=18, fontweight='bold')
    ...
    # 颜色和范围配置
    cmap_str = 'RdYlGn_r' if metric_choice == '中标概率' else 'YlOrRd'
...
def run_win_rate_matrix_analysis_calc(
    ...
    """运行中标概率矩阵分析的计算流程，返回结果 DataFrame 和热力图 Figure"""
    df = calc_win_rate_matrix_data(
        cap_start, cap_end, cap_steps,
        price_start, price_end, price_steps,
        k_start, k_end, k_steps,
        n_sims, 
    ...
    fig = plot_win_rate_matrix_data(df, metric_choice)
    return df, fig
...
with gr.Blocks(title="储能市场竞争蒙特卡洛模拟（2025细则+优胜劣汰+容量扫描）") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("📊 单次模拟"):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    N_day_std = gr.Slider(10, 200, 50, step=10, label="日响应次数标准差")
                    d_mean = gr.Slider(0.3, 0.8, 0.5, step=0.05, label="调节深度均值")
                    d_std = gr.Slider(0.05, 0.3, 0.1, step=0.01, label="调节深度标准差")
        ...
        # 标签页6：出清边界分析
        with gr.Tab("🎯 出清边界分析"):
            gr.Markdown("分析在供过于求时，电站需要的**性能K值**和**报价**底线，直观展示被淘汰者的特征.")
            boundary_btn = gr.Button("生成出清边界散点图", variant="primary")
            boundary_plot = gr.Plot()
            boundary_summary = gr.Markdown()
        
        #标签页7：三维全空间中标概率测算
        with gr.Tab("🎲 三维全空间中标概率测算"):
            gr.Markdown("自定义遍历【总申报容量】×【电站性能K】×【申报报价】三个维度的正交空间，生成各切面的指标网格图。")
            
            metric_radio = gr.Radio(choices=["中标概率", "期望收益"], value="中标概率", label="展示指标", interactive=True)
            grid_data_state = gr.State()
            
            with gr.Row():
                with gr.Column():
                    grid_cap_start = gr.Number(value=1200, label="起步仿真容量(MW)")
                    grid_cap_end = gr.Number(value=2500, label="终端仿真容量(MW)")
            ...
            gen_grid_btn = gr.Button("生成三维切面矩阵", variant="primary")
            grid_plot_out = gr.Plot()
    ...
    sens_btn.click(
        ...
        # 绑定价格敏感性分析按钮上方或下方，增加出清边界按钮绑定
    boundary_btn.click(
        fn=run_clearing_boundary_analysis,
        inputs=[
            total_cap, max_demand, dist_demand_ratio,
            bid_mean, bid_std_ratio,
            k_mean, k_std, m_mean, m_std, m_competition_factor, m_max,
        ...
        outputs=[boundary_plot, boundary_summary]
    )

    # 绑定指标切换事件
    metric_radio.change(
        fn=plot_win_rate_matrix_data,
        inputs=[grid_data_state, metric_radio],
        outputs=[grid_plot_out]
    )
    ...
    gen_grid_btn.click(
        ...
        inputs=[
            ...
            max_demand, dist_demand_ratio, grid_bid_mean, bid_std_ratio,
            grid_k_mean, k_std, m_mean, m_std, m_competition_factor, m_max,
            N_day_mean, N_day_std, d_mean, d_std,
            U_x, U_y, cap_ratio_upper, cap_ratio_lower, Q_lb, price_min, price_max
        ],
        outputs=[grid_data_state, grid_plot_out]
    )
