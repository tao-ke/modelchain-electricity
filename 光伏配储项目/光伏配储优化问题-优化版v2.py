...
# 定义优化函数
def optimize_single_day(price, day_index):
    """优化单天的储能调度"""
    print(f"\n{'=' * 60}")
    print(f"正在优化第 {day_index + 1} 天")
    print("=" * 60)
...
for day_idx in range(num_days):
    ...
    # 优化当天
    prob, result = optimize_single_day(price, day_idx)
    ...
    if pulp.LpStatus[prob.status] == "Optimal":
        ...
        for k in range(1, num + 1):
            ...
            if k < num:
                if efficiency * cumulative_charge < cumulative_discharge - tolerance:
                    balance_constraint_violations += 1
            else:
                if abs(efficiency * cumulative_charge - cumulative_discharge) > tolerance:
                    balance_constraint_violations += 1
        ...
        all_summaries.append({
            ...
            '初始电量(kWh)': f"{initial_soc:.2f}",
            '最终电量(kWh)': f"{soc_values[-1]:.2f}",
...
# 提取日期列作为新列名
date_columns = summary_df['日期'].values
# 删除日期列
